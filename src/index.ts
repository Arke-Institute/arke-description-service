 ...
 *
 * Cloudflare Worker that performs bottom-up LLM-based summarization
 * of directory hierarchies for the Arke Institute photo archive.
 */
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname !== '/summarize') {
      return new Response(
        JSON.stringify({
          error: `Not found. Available endpoints: POST /summarize, POST /extract-metadata, POST /validate-metadata`
        }),
        {
          status: 404,
          headers: {
